...
//Valor de pi para as rotacoes em radianos
...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "INF01047 - 270139 - Cleiber Rodrigues", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
            ...
            PushMatrix(model); // Guardamos matriz model atual na pilha
                ...
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    ...
                    PushMatrix(model); // Guardamos matriz model atual na pilha
                        ...
                        //Aqui iremos transladar um pouco a posicao a partir do antebraco para o desenho da mao direita
                        //E em seguida iremos aplicar a matrix_scale de forma a diminuir o tamanho do cubo para fazer o desenho da mao
                        PushMatrix(model);
                            model = model * Matrix_Translate(0.0f, -1.09f, 0.0f);
                            PushMatrix(model);
                                model = model * Matrix_Scale(1.0f, 0.2f, 1.0f);
                                glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
        ...
        //Aqui iremos primeiro tentar fazer a cabeca com base no codigo do braco, porem com a rotacao em 180 no cubo
        PushMatrix(model); // Guardamos matriz model atual na pilha
            model = model * Matrix_Rotate_Z(pi);
            ...
            PushMatrix(model); // Guardamos matriz model atual na pilha
                model = model // Atualizamos matriz model (multiplicação à direita) com a rotação da cabeca
                      ...
                      * Matrix_Rotate_X(g_AngleX); // PRIMEIRO rotação X de Euler
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model * Matrix_Scale(0.3f, 0.3f, 0.2f); // Atualizamos matriz model (multiplicação à direita) com um escalamento da cabeca
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                    DrawCube(render_as_black_uniform); // #### CABECA // Desenhamos a cabeca
        ...
        //Aqui iremos tentar reproduzir o braco esquerdo completo tentando espelhar as coordenadas do braco direito
        PushMatrix(model); // Guardamos matriz model atual na pilha
            model = model * Matrix_Translate(+0.55f, 0.0f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com uma translação para o braço direito
            PushMatrix(model); // Guardamos matriz model atual na pilha
                model = model // Atualizamos matriz model (multiplicação à direita) com a rotação do braço direito
        ...
        //Agora iremos tentar desenhar as pernas seguindo a logica dos bracos (porem com as devidas translacoes)
        PushMatrix(model); // Guardamos matriz model atual na pilha
            model = model * Matrix_Translate(-0.15f, -1.05f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com uma translação para o braço direito
            PushMatrix(model); // Guardamos matriz model atual na pilha
                PushMatrix(model); // Guardamos matriz model atual na pilha
                ...
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model * Matrix_Translate(0.0f, -0.65f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com a translação do antebraço direito
                    PushMatrix(model); // Guardamos matriz model atual na pilha
                        model = model * Matrix_Scale(0.25f, 0.6f, 0.2f); // Atualizamos matriz model (multiplicação à direita) com um escalamento do antebraço direito
                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                        ...
                        //E em seguida iremos aplicar a matrix_scale de forma a diminuir o tamanho do cubo para fazer o desenho da mao
                        PushMatrix(model);
                            model = model * Matrix_Translate(0.0f, -1.09f, 0.25f);
                            PushMatrix(model);
                                model = model * Matrix_Scale(1.3f, 0.2f, 2.0f);
                                glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
        ...
        //Agora iremos fazer a perna direita seguindo a mesma logica da perna esquerda
        PushMatrix(model); // Guardamos matriz model atual na pilha
            model = model * Matrix_Translate(+0.25f, -1.05f, 0.0f); // Atualizamos matriz model (multiplicação à esquerda) com uma translação para a perna esquerda
            PushMatrix(model); // Guardamos matriz model atual na pilha
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model * Matrix_Scale(0.3f, 0.6f, 0.2f); // Atualizamos matriz model (multiplicação à esquerda) com um escalamento da perna esquerda
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                    DrawCube(render_as_black_uniform); // #### PE ESQUERDO // Desenhamos o pe esquerdo
                PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model * Matrix_Translate(0.0f, -0.65f, 0.0f); // Atualizamos matriz model (multiplicação à esquerda) com a translação da canela esquerda
                    PushMatrix(model); // Guardamos matriz model atual na pilha
                        model = model * Matrix_Scale(0.25f, 0.6f, 0.2f); // Atualizamos matriz model (multiplicação à direita) com um escalamento da canela esquerda
                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                        ...
                        PushMatrix(model);
                            ...
                            PushMatrix(model);
                                model = model * Matrix_Scale(1.3f, 0.2f, 2.0f);
                                glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                                DrawCube(render_as_black_uniform); // #### PE ESQUERDA // Desenhamos o pe esquerdo
                            PopMatrix(model);
                        PopMatrix(model);
